...
/// Tiny local growable list used instead of std.ArrayList to avoid
/// stdlib API churn between Zig versions.
fn SimpleList(comptime T: type) type {
    return struct {
        allocator: Allocator,
        items: []T,
        len: usize,
        ...
        pub fn init(allocator: Allocator) Self {
            return .{
                .allocator = allocator,
                .items = (&[_]T{})[0..0], // zero-length slice
                .len = 0,
            };
        }
        ...
        pub fn append(self: *Self, value: T) !void {
            if (self.len == self.items.len) {
                const new_capacity: usize =
                    if (self.items.len == 0) 8 else self.items.len * 2;

                const new_buf = try self.allocator.alloc(T, new_capacity);
                mem.copyForwards(T, new_buf[0..self.len], self.items[0..self.len]);
                if (self.items.len != 0) {
                    self.allocator.free(self.items);
                }
                self.items = new_buf;
        ...
        pub fn toOwnedSlice(self: *Self) []T {
            const slice = self.items[0..self.len];
            // Hand ownership of the buffer to the caller; reset ourselves to empty.
            self.items = (&[_]T{})[0..0];
            self.len = 0;
            return slice;
        }
...
fn parseElement(parser: *Parser, alloc: Allocator, comptime kind: ElementKind) !?*Element {
    ...
    var attributes = SimpleList(Attribute).init(alloc);
    defer attributes.deinit();

    var children = SimpleList(Content).init(alloc);
