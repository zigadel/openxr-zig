...
fn SimpleList(comptime T: type) type {
    return struct {
        ...
        /// Caller takes ownership of the slice.
...
pub fn parseTypedef(allocator: Allocator, xctok: *XmlCTokenizer, ptrs_optional: bool) !registry.Declaration {
    ...
    _ = switch (first_tok.kind) {
        ...
        .type_name => {
            ...
            std.debug.print("unexpected first token in typedef: {any}\n", .{first_tok.kind});
            return error.InvalidSyntax;
        },
    };
}
...
fn parseFnPtrSuffix(allocator: Allocator, xctok: *XmlCTokenizer, return_type: TypeInfo, ptrs_optional: bool) !?Declaration {
    ...
    // The current case is that we can't have an array of function pointers as
    // parameters to a function pointer,
    // so a small growable list backed by the same allocator is good enough.
    var params = SimpleList(registry.Command.Param).init(allocator);
    try params.append(.{
